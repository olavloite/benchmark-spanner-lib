...
public class BenchmarkRunner
{
    ...
    private class PointSelectRunner
    {
        ...
        public void RunPointSelect()
        {
            ...
            while (DateTime.Now - start < _duration)
            {
                ...
                _measurements.Add(MeasurePointSelect());
            }
        }

        private TimeSpan MeasurePointSelect()
        {
            return Utils.Measure(PointSelect);
        }
    ...
    public Measurement MeasureExecuteQuery(int numIterations, int numRows)
    {
        ...
        {
            durations.Add(MeasureExecuteQuery(connection, numRows));
        }
        return Utils.ExtractMeasurement(durations);
    }

    public TimeSpan MeasureExecuteQuery(DbConnection connection, int numRows = 1000)
    {
        return Utils.Measure(() => ExecuteQuery(connection, numRows));
    }

    public void ExecuteQuery(DbConnection connection, int numRows = 1000)
    {
        using var cmd = connection.CreateCommand();
